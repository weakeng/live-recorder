...
class InkeLive extends Live {
    ...
    public static readonly SITE_CODE = "InkeLive";
    ...
    public async refreshRoomData() {
        ...
        let res = await Http.request({url: url}).catch((err) => {
            throw `获取房间信息失败,网络异常,${InkeLive.SITE_NAME}(${this.roomUrl})`;
        });
        if (res['error_code'] === 0) {
            ...
            this.setLiveStatus(false);
            throw `获取房间信息失败,主播未开播或房间地址有误,${InkeLive.SITE_NAME}(${this.roomUrl})`;
        }
    }
    ...
    public async getLiveUrl() {
        ...
        let res = await Http.request({url: url}).catch(() => {
            throw `获取直播源信息失败,网络异常,${InkeLive.SITE_NAME}(${this.roomUrl})`;
        });
        if (res['error_code'] === 0) {
            let liveList = [];
            let item = {
            ...
            throw `获取直播源信息失败！主播未开播或房间地址有误,${InkeLive.SITE_NAME}(${this.roomUrl})`;
        }
    }
}
